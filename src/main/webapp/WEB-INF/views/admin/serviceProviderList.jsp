...
<head>
    ...
    <title>List of ServiceProvider</title>
</head>
...
<body>
	...
	<div class="container">
		<table class="table">
			<caption>List of Service Provider</caption>
			<thead>
				<tr>
					<th scope="col">Sl.</th>
			...
			<tbody>
				<c:forEach var="sp" items="${sp}" varStatus="myIndex">
					<tr>
						<th scope="row">${myIndex.index+1}</th>
						<td>${sp.companyName}</td>
						<td>${sp.emailId}</td>
						<td>${sp.phoneNo}</td>
						<td>${sp.serviceProviderType}</td>
						<td><a href="sp-details?id=${sp.userId}">View
								Profile</a></td>
					</tr>
				</c:forEach>
			</tbody>
