...
<body>
    ...
    <div class="container">
    	<c:if test = "${cnfOrder.isEmpty()}">
    			<div class="alert alert-danger d-flex justify-content-center mt-5">
            		<h3>No Records</h3>
    			</div> 
        </c:if>
		<c:forEach var="cnfOrder" items="${cnfOrder}">
			<section class="myOrderContainer">
				<div class="row">
					<div class="col-sm">Order Id : ${cnfOrder.orderId}</div>
					<div class="col-sm">Service Type : ${cnfOrder.serviceType}</div>
					<div class="col-sm">Price : ${cnfOrder.price}</div>
					<div class="col-sm">Service Provider Id : ${cnfOrder.serviceProviderId}</div>
					<div class="col-sm">Customer Id : ${cnfOrder.customerId}</div>
					<div class="col-sm">Service Date : ${cnfOrder.serviceDate}</div>
					<div class="col-sm"><a class="btn btn-primary" href="/admin/address?shno=${cnfOrder.sourceAddHouseNo}&dhno=${cnfOrder.destAddHouseNo}">View Address</a></div>
					<div class="col-sm"><a class="btn btn-primary" href="/admin/itemlist/${cnfOrder.orderId}">Items List</a></div>
				</div>
			</section>
		</c:forEach>
	</div>
